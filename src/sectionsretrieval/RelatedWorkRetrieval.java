/*
 * The script retrieves the related work section by using the Scanner method which scans the input xml file line by line
 * and searches for the keyword "Related Work" or "Background" or "Previous Work" or "Preliminaries".
 * If the line starts or ends with either of these key words and as long as
 ...
 */
public class RelatedWorkRetrieval {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String path = "../relatedwork.txt";
        for (File file : files) {
            Scanner scan = new Scanner(file, "UTF-8");
            String contents = "";
            while (scan.hasNextLine()) {
                String myLine = scan.nextLine();
                if (myLine.endsWith("Related Work") || myLine.endsWith("Background") || myLine.endsWith("Previous Work") || myLine.endsWith("Preliminaries")) {
                    while (!(myLine.startsWith("<sectionHeader"))&& scan.hasNextLine()) {
                        ...
                        if (myLine.startsWith("<bodyText confidence") || myLine.startsWith("</bodyText")) {
                            ...
                            continue; 
                         } else if (myLine.contains("<affiliation confidence") || myLine.contains("</affiliation>")) {
                            continue;  
